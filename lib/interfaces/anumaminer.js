...
module.exports = class Simulator extends Daemon {
	...
	async start() {
		...
		let defaults = {
			...
			// dbuser: 	'anuma', // Database user
			// dbpass: 	'anuma', // Database password
...
//		let {config:daemons} = await this.get("get-modules-config");
		...
		if(!args.miningaddr) {
			args.miningaddr = 'anumatest:qpjmmd8fc20c6mf5zxr6qax0369g0aufyg6aqcel9u';
			//args.miningaddr = 'anuma:qqg767gkznmsuah5xeer9f72lwqu642nqvlmujtpmk';
			//args.miningaddr = 'anuma:qr35ennsep3hxfe7lnz5ee7j5jgmkjswsslwxj42ta';
		}		

		//if(process.env['ANUMA_JSON_RPC'])
		...
		//console.log("++++++++++++++++ ANUMAMINER VSARGS")

		await this.VSARGS([this.getDeployableBinary('anumaminer'), '--version']);

		args = Object.entries(args);

		const flags = utils.args();
		//console.log("++++++++++++++++ ANUMAMINER FLAGS",flags)

		this.options = {
			args : () => {
				// this.log(`--addresslist=${configFile}`,`--cert=${rpcCertFile}`);
				let _args_ = [
					this.getBinary('anumaminer'),
			...
			stdout : (data) => { 
				...
				// // console.log("anumaminer match:", match);
		...
		//console.log("ANUMAMINER STATUS:",this.manager.enableMining,flags.mine);
		if(this.manager.enableMining || flags.mine) {
			dpc(0, () => {
				//console.log("----------------- ANUMAMINER STARTING")
				this.run()
			})
		} else {
			//console.log("----------------- ANUMAMINER NOT RUNNING")
			this.state = 'disabled';
			dpc(0, () => {
				this.writeToSink('\r\n\r\nAnumaminer is disabled. Please use Enable Mining option to start it.\r\n\r\n');
			})
		}
