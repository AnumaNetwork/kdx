...
//TODO
window.PWA_MODULES={};
window.PWA_MODULES["@anuma-network/wallet-pwa"] = "N/A";
window.PWA_MODULES["@aspectron/flow-ux"] = "N/A";
window.PWA_MODULES["@anuma-network/ux"] = "N/A";
...
//			['Mosquitto','EDL-V10 EPL-V10','Copyright (c) 2007, Eclipse Foundation, Inc. and its licensors'],
		...
		return html`
		<flow-caption-bar
			logo="/resources/images/anuma-logo-light-bg.png">KDX</flow-caption-bar>
		<tab-content for="home">
			...
			<div id='anuma-resources'>

				<flow-expandable>
					<div slot="title" is="i18n-div" caption>ANUMA RESOURCES</div>
					<ul style="font-size: 12px;">
						<li>
							<flow-shell-link href="https://docs.kas.pa/anuma/about-anuma/get-started">
			...
			<flow-form-control icon="fal:copyright">
				<flow-i18n>KDX &amp; Anuma Copyright (c) 2020 Anuma Developers<br/>
		...
		<tab-content for="wallet" class="wallet" data-active-display="flex">
			<anuma-wallet .walletMeta='${walletMeta}' hideNetwork hidefaucet 
				_hideQRScanner hideopenwalletlogo></anuma-wallet>
		</tab-content>
...
//		this.checkForUpdates().then();
	...
	async initManager(){
		...
		manager.on('sync-status', (data) => {
			//console.log("sync-status:data", data);
			let wallet = this.wallet || this.qS('anuma-wallet');
	...
	async initWallet() {
		let wallet = this.qS('anuma-wallet');
		this.wallet = wallet;
	...
	setTheme(theme){
		...
		if(this.caption)
			this.caption.logo = `/resources/images/anuma-logo-${theme}-bg.png`
		...
		document.body.dispatchEvent(new CustomEvent("flow-theme-changed"));
		this.querySelector("anuma-wallet")?.requestUpdate("theme", null)
	}
	initCaption(){
		...
		this.caption.logo = `/resources/images/anuma-logo-${this.theme||"light"}-bg.png`;
	...
	async initTemplates() {
		...
		if(!this.tpl_template) {
			...
			let miner = Object.keys(config.modules).filter(v=>/^(anuma|gpu)miner/.test(v));
			if(!miner.length)
...
Useful resources:
- Anuma Documentation: https://github.com/AnumaNetwork/docs 
- Anuma Discord: https://discord.gg/vMT39xB
- Anuma GitHub: https://github.com/AnumaNetwork/
- KDX GitHub: https://github.com/AnumaNetwork/kdx
